...
const CONNECTION_STRING = new Client(process.env.DATABASE_URL || 'postgres://localhost:5432/phenomena-dev');
...
async function getOpenReports() {
  ...
  SELECT *
  FROM comments
  WHERE "reportId" IN (${openReports.map((report) => report.id).join(', ')});
  `);
  ...
  openReports.forEach((report) => {
    const reportComments = openComments.filter(
...
async function createReport(reportFields) {
  ...
  try {
    ...
    console.log("createReport report", report)
